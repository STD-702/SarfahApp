...
class ManageStudentsActivity : AppCompatActivity() {
    ...
    private fun setupBottomNavigation() {
        ...
        bottomNavigation.setOnNavigationItemSelectedListener { menuItem ->
            when (menuItem.itemId) {
                ...
                R.id.nav_create_delegate -> {
                    startActivity(Intent(this, CreateDelegateActivity::class.java))
                    true
                }
    ...
    private fun fetchParentStudents() {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    Toast.makeText(this@ManageStudentsActivity, "Failed to load students: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                response.use {
                    if (!it.isSuccessful) {
                        runOnUiThread {
                            Toast.makeText(this@ManageStudentsActivity, "Unexpected response: ${it.code}", Toast.LENGTH_SHORT).show()
                        }
                        return
                    }
                    ...
                    runOnUiThread {
                        if (jsonResponse.getString("status") == "success") {
                            ...
                            Toast.makeText(this@ManageStudentsActivity, jsonResponse.getString("message"), Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
    ...
    private fun handleStudentData(studentsJsonArray: JSONArray) {
        ...
        for (i in 0 until studentsJsonArray.length()) {
            ...
            val student = Student(
                ...
                studentJson.optString("class", ""),
                studentJson.optString("grade", ""),
            )
    ...
    private fun setupStudentAdapter(students: List<Student>) {
        val studentsAdapter = ParentStudentsAdapter(
    ...
    private fun filterStudents(query: String?) {
        (rvStudents.adapter as? ParentStudentsAdapter)?.filter(query)
    }
    ...
    private fun sendPickupRequest(studentId: Int, pickupTime: String) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    Toast.makeText(this@ManageStudentsActivity, "Failed to send request: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    private fun requestEarlyLeave(student: Student) {
        if (!isInSchoolPremises) {
            Toast.makeText(this, "You must be within the school premises to send an early leave request.", Toast.LENGTH_LONG).show()
            return
        }

        val currentHour = Calendar.getInstance().get(Calendar.HOUR_OF_DAY)

        if (currentHour in 1..23) {
            showEarlyLeaveDialog(student)
        } else {
            Toast.makeText(this, "Early leave requests can only be sent from 7 AM to 11:59 AM.", Toast.LENGTH_SHORT).show()
        }
    }

    private fun showEarlyLeaveDialog(student: Student) {
        val builder = AlertDialog.Builder(this)
        builder.setTitle("Early Leave Request for ${student.firstName} ${student.lastName}")

        val view = layoutInflater.inflate(R.layout.dialog_request_early_leave, null)
        builder.setView(view)

        val etLeaveMessage = view.findViewById<EditText>(R.id.etLeaveMessage)

        builder.setPositiveButton("Send Request") { _, _ ->
            val currentTimeString = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(Date())
            val leaveMessage = etLeaveMessage.text.toString()
            sendEarlyLeaveRequest(student.studentId, currentTimeString, leaveMessage)
        }

        builder.setNegativeButton("Cancel") { dialog, _ -> dialog.dismiss() }
        builder.show()
    }
    private fun sendEarlyLeaveRequest(studentId: Int, pickupTime: String, message: String) {
        val url = "http://192.168.3.65/dashboard/sarfah1.11/send_request.php"
        val formBody = FormBody.Builder()
            .add("student_id", studentId.toString())
            .add("pickup_time", pickupTime)
            .add("status", "pending")
            .add("request_msg", message)
            .build()
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    Toast.makeText(this@ManageStudentsActivity, "Failed to send early leave request: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
